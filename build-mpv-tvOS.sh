...
-I$CWD/FFmpeg-tvOS/thin/$ARCH/include"
    export LDFLAGS="-isysroot $SDKPATH -arch $ARCH -Wimplicit-function-declaration,$DEPLOYMENT_TARGET -lbz2 \
-L$CWD/FFmpeg-tvOS/thin/$ARCH/lib"

#Wimplicit-function-declaration keep only
#export CC="clang -target x86_64-apple-darwin"
#export CXX="clang++ $LDFLAGS"
    echo $CFLAGS
    echo $LDFLAGS

#--enable-videotoolbox-gl
    OPTION_FLAGS=" --disable-cplayer --disable-lcms2 --disable-lua --enable-libmpv-static --enable-ios-gl --enable-gl --disable-libass --enable-lgpl \
